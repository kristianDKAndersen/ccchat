...
export function upsertAgent({ name, projectPath, rooms, setOnline = true }) {
  ...
  `).run(name, hash, projectPath, JSON.stringify(existingRooms), onlineValue, projectPath, JSON.stringify(existingRooms), onlineValue);
}
...
export function getUnreadCountAllRooms(agentName, projectPath) {
  ...
  const stmt = d.prepare('SELECT last_id FROM read_cursors WHERE agent_name = ? AND project_hash = ? AND room = ?');
  const countStmt = d.prepare('SELECT COUNT(*) AS cnt FROM messages WHERE room = ? AND id > ? AND from_agent != ?');

  for (const room of rooms) {
    const cursor = stmt.get(agentName, hash, room);
    const lastId = cursor ? cursor.last_id : 0;
...
export function initCursorIfNew(agentName, projectPath, room) {
  const d = getDb();
  const hash = projectHash(projectPath);
  const existing = d.prepare('SELECT 1 FROM read_cursors WHERE agent_name = ? AND project_hash = ? AND room = ?').get(agentName, hash, room);
  if (!existing) {
    const maxId = d.prepare('SELECT COALESCE(MAX(id), 0) AS id FROM messages WHERE room = ?').get(room);
    d.prepare(`
      INSERT INTO read_cursors (agent_name, project_hash, room, last_id) VALUES (?, ?, ?, ?)

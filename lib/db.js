...
// CCCHAT_DB_PATH allows tests to use a temp DB without touching the real one.
// Re-evaluated each time getDb() is called (db=null after closeDb()).
function getDbPath() {
  return process.env.CCCHAT_DB_PATH || join(DB_DIR, 'ccchat.db');
}
...
export function getDb() {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_room_phases_room ON room_phases(room, id);
  `);
  ...
  if (!agentCols.includes('current_room')) {
    db.exec(`ALTER TABLE agents ADD COLUMN current_room TEXT NOT NULL DEFAULT 'lobby'`);
    db.exec(`UPDATE agents SET current_room = 'lobby'`);
  }
...
export function upsertAgent({ name, projectPath, currentRoom, setOnline = true, ...unknownParams }) {
  const unknownKeys = Object.keys(unknownParams);
  if (unknownKeys.length > 0) {
    throw new Error(`upsertAgent: unknown param(s): ${unknownKeys.join(', ')} — valid params are: name, projectPath, currentRoom, setOnline`);
  }
  const d = getDb();
  ...
  const existing = d.prepare('SELECT online FROM agents WHERE name = ? AND project_hash = ?').get(name, hash);
  ...
  // Read-only / hook-path upsert: DO update last_seen so hook activity extends
  // the heartbeat. Without this, agents who only poll without sending expire
  // after 10 min via getOnlineAgents() auto-expiry and drop out silently.
  d.prepare(`
    INSERT INTO agents (name, project_hash, project_path, current_room, online, last_seen)
    VALUES (?, ?, ?, ?, ?, datetime('now'))
  ...
  `).run(name, hash, projectPath, room, onlineValue, projectPath, room, onlineValue);
}
...
export function setCurrentRoom(name, projectPath, room) {
  const knownRooms = new Set([...getAllRooms(), ...PROTECTED_ROOMS]);
  if (!knownRooms.has(room)) {
    throw new Error(`Room '${room}' does not exist. Use --create to create it.`);
  }
  ...
  d.prepare(`
    UPDATE agents SET current_room = ?, last_seen = datetime('now')
...
export const PROTECTED_ROOMS = ['lobby'];

export function leaveCurrentRoom(name, projectPath) {
  const d = getDb();
  const hash = projectHash(projectPath);

  const existing = d.prepare('SELECT current_room FROM agents WHERE name = ? AND project_hash = ?').get(name, hash);
  if (!existing) return { alreadyInLobby: true };

  const oldRoom = existing.current_room || 'lobby';
  if (oldRoom === 'lobby') return { alreadyInLobby: true };

  d.prepare(`
    UPDATE agents SET current_room = 'lobby', rooms = '["lobby"]', last_seen = datetime('now')
    WHERE name = ? AND project_hash = ?
  `).run(name, hash);

  resetLobbyCursor(name, projectPath);

  emitEvent('room:leave', { agent: name, room: oldRoom });

  return { room: oldRoom, hash };
}

export function getAgentRoom(name, projectPath) {
  const d = getDb();
  const hash = projectHash(projectPath);
  const row = d.prepare('SELECT current_room FROM agents WHERE name = ? AND project_hash = ?').get(name, hash);
  return (row && row.current_room) || 'lobby';
}
...
// Scan ALL rooms for urgent/DM/@mention signals the agent hasn't read.
// Does NOT advance any cursor — purely observational.
export function getCrossRoomSignals(agentName, projectPath) {
  const d = getDb();
  const hash = projectHash(projectPath);

  const rows = d.prepare(`
    SELECT m.room, m.id, m.type, m.from_agent, m.to_agent, m.content, m.metadata
    FROM messages m
    LEFT JOIN read_cursors rc ON rc.agent_name = ? AND rc.project_hash = ? AND rc.room = m.room
    WHERE m.id > COALESCE(rc.last_id, 0)
      AND m.from_agent != ?
    ORDER BY m.id ASC
    LIMIT 50
  `).all(agentName, hash, agentName);

  const signals = [];
  for (const row of rows) {
    const meta = parseMeta(row.metadata);
    const isUrgent = meta.priority === 'urgent';
    const isDM = row.to_agent === agentName;
    const isMention = Array.isArray(meta.mentions) && meta.mentions.includes(agentName);

    if (!isUrgent && !isDM && !isMention) continue;

    const tags = [];
    if (isUrgent) tags.push('URGENT');
    if (isDM) tags.push('DM');
    if (isMention) tags.push('@you');
    signals.push({ room: row.room, id: row.id, type: row.type, from_agent: row.from_agent, content: row.content, tags });
  }
...
export function createRoom(agentName, projectPath, roomName) {
  const ROOM_RE = /^[a-z0-9][a-z0-9-]*$/;
  if (!ROOM_RE.test(roomName)) {
    throw new Error(`Invalid room name '${roomName}' — must be lowercase alphanumeric + hyphens only`);
  }

  const existing = getAllRooms();
  if (existing.includes(roomName)) {
    // Already exists — nothing to do. Caller can still join.
  ...
  // Rooms have no separate table — a room exists when it has messages.
  // Bootstrap it with a system message, matching dashboard /api/rooms/create.
  upsertAgent({ name: agentName, projectPath, currentRoom: roomName });
  initCursorIfNew(agentName, projectPath, roomName);
  const result = d.prepare(`
    INSERT INTO messages (type, from_agent, from_project, room, content, metadata)
    VALUES ('system', ?, ?, ?, ?, ?)
  `).run(agentName, hash, roomName, `Room #${roomName} created by ${agentName}`, JSON.stringify({ mentions: [], priority: 'normal' }));
  const maxId = d.prepare('SELECT MAX(id) AS id FROM messages WHERE room = ?').get(roomName).id;
  updateCursor(agentName, projectPath, roomName, maxId);

  return { created: true, messageId: result.lastInsertRowid };
}
...
export function deleteRoom(room) {
  ...
  const d = getDb();
  d.transaction(() => {
    // 1. Delete plan_tasks for plans tied to this room (by room or source message)
    d.prepare(`
      DELETE FROM plan_tasks WHERE plan_id IN (
        SELECT id FROM plans WHERE room = ? OR source_message_id IN (
          SELECT id FROM messages WHERE room = ?
        )
      )
    `).run(room, room);

    // 2. Delete plans tied to this room
    d.prepare(`
      DELETE FROM plans WHERE room = ? OR source_message_id IN (
        SELECT id FROM messages WHERE room = ?
      )
    `).run(room, room);

    // 3. Nullify parent_id on cross-room replies so messages in other rooms survive
    d.prepare(`
      UPDATE messages SET parent_id = NULL
      WHERE parent_id IN (SELECT id FROM messages WHERE room = ?)
    `).run(room);

    // 4. Now safe to delete the room's messages
    d.prepare('DELETE FROM messages WHERE room = ?').run(room);

    // 5. Clean up ancillary room records
    d.prepare('DELETE FROM planner_locks WHERE room = ?').run(room);
    d.prepare('DELETE FROM room_phases WHERE room = ?').run(room);
    d.prepare('DELETE FROM read_cursors WHERE room = ?').run(room);

    // 6. Reset any agent whose current_room is the deleted room back to lobby
    d.prepare(`
      UPDATE agents SET current_room = 'lobby', rooms = '["lobby"]'
      WHERE current_room = ?
    `).run(room);
  })();
}
...
export function setPhase(room, phase, agentName, notes = null) {
  const d = getDb();
  d.prepare(`
    INSERT INTO room_phases (room, phase, set_by, notes)
    VALUES (?, ?, ?, ?)
  `).run(room, phase, agentName, notes || null);
}

export function getPhase(room) {
  const d = getDb();
  return d.prepare(`
    SELECT * FROM room_phases WHERE room = ? ORDER BY id DESC LIMIT 1
  `).get(room) || null;
}

export function getPhaseHistory(room, limit = 20) {
  const d = getDb();
  return d.prepare(`
    SELECT * FROM room_phases WHERE room = ? ORDER BY id DESC LIMIT ?
  `).all(room, limit).reverse();
}

export function getConsensusMessages(room, topic = null) {
  const d = getDb();
  if (topic) {
    return d.prepare(`
      SELECT * FROM messages
      WHERE room = ?
        AND json_extract(metadata, '$.consensus_signal') IS NOT NULL
...
export function getStaleUnansweredQuestions(room, ageMinutes = 15) {
  const d = getDb();
  return d.prepare(`
    SELECT * FROM messages
    WHERE room = ?
      ...
      AND id NOT IN (SELECT DISTINCT parent_id FROM messages WHERE parent_id IS NOT NULL)
    ORDER BY id ASC
  `).all(room, ageMinutes);
}
...
export function getActivePlanInRoom(room) {
  const d = getDb();
  return d.prepare("SELECT * FROM plans WHERE room = ? AND status = 'active' LIMIT 1").get(room) || null;
}
...
export const PHASE_ALLOWLIST = {
  'plan:create':   new Set(['brainstorm', 'draft', null]),
  'plan:activate': new Set(['draft', null]),
  'task:claim':    new Set(['execute', null]),
  'send:agree':    new Set(['peer_review', 'review', null]),
  'send:disagree': new Set(['peer_review', 'review', null]),
};

export function checkPhaseAllowed(room, operation) {
  const allowed = PHASE_ALLOWLIST[operation];
  if (!allowed) throw new Error(`Unknown operation: ${operation}`);
  const record = getPhase(room);
  const phase = record ? record.phase.toLowerCase() : null;
  if (allowed.has(phase)) return { allowed: true };
  return {
    allowed: false,
    current: phase,
    message: `Cannot perform '${operation}' in phase '${phase}'. Allowed: ${[...allowed].filter(p => p !== null).join(', ')}`,
  };
}

...
3. **Stop ALL /loop polling** (critical): Use `CronList` to list all cron jobs, then `CronDelete` for every job whose prompt contains `/ccchat`. There may be multiple — kill them all. Do NOT use TaskList/TaskStop (those are for tasks, not cron jobs).
